...
@section('content')
{!!Form::open( ['route' => 'admin.updateTechnical' ,'class'=>'form phone_validate', 'method' => 'Post', 'enctype'=>"multipart/form-data",'class'=>'form-horizontal','files' => true]) !!}
                        @csrf
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box">
            ...
            <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap">
                ...
                <tr role="row">
                    <th class="sorting_asc" tabindex="0" aria-controls="datatable-buttons" rowspan="1"
                        colspan="1" aria-sort="ascending">Technical Daily Report
                    </th>
                    <th class="sorting_asc" tabindex="0" aria-controls="datatable-buttons" rowspan="1"
            ...
            <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap">
                   ...
                   <tbody>
                   @if(isset($rides_down_items))
                   @foreach ($rides_down_items as $item)

                            <tr role="row" class="odd" id="row-{{ $item->id }}">
                           <td>
                           {{$item->ride->name ??''}}
                               <input type="hidden" name="ride_down_id[]" class="ride-down-id" value="{{ $item->ride_down_id }}">
                           </td><td>
                          <label>
                           ...
                           <td>
                               <label>
                               {!! Form::date('date_expected_open[]',$item->date_expected_open, array('class' => 'form-control date-expected-open')) !!}
                               </label>
                           </td>
                           <td>
                               {!! Form::text('lead_name[]',$item->lead_name, array('class' => 'form-control lead-name')) !!}
                           </td>
                           <td>
                               {!! Form::textArea('ride_down_comment[]',$item->comment, array('class' => 'form-control ride-down-comment summernote')) !!}
                           </td>
                       </tr>
                        @endforeach
                     @endif
                       @foreach ($rides as $item)
                       @if(!in_array($item->id, $rides_down_items_array))
...
@push('scripts')
    ...
    $(document).ready(function () {
        $('.save_btn').on('click',function (e) {
            ...
            $.ajax({
                url: "{{ route('admin.updateTechnical') }}",
                type: 'POST',
                data:{
                    "_token":"{{ csrf_token() }}",
                    answer: answer,
                ...
                success: function(response)
                {
                    if(response.success){
                    swal({
                         title: "Technical Report Updated successfully",
                         icon: "success",
                         buttons: ["Ok"]
                          }); 
                        ...
                        swal({
                         title: "Technical Report Already Exist !",
                         icon: "danger",
                         buttons: ["Ok"],
